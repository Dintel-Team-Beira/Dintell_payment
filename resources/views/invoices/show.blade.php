    ...
    <div class="flex space-x-3">
        ...
        @if ($invoice->status == 'paid')
            <a href="{{ route('receipts.download-pdf', $invoice->latestReceipt) }}"
                class="inline-flex items-center px-4 py-2 text-green-600 transition-colors rounded-lg bg-green-50 hover:bg-green-100 hover:text-green-700"
                title="Baixar PDF">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M12 10v6m0 0l-3-3m3 3l3-3M3 17V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2z" />
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-red-700">
                            <p>Esta factura venceu em {{ $invoice->due_date->format('d/m/Y') }}
                                ({{ $invoice->due_date->diffForHumans() }}). Entre em contato com o cliente para
                                regularizar o pagamento.</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white border border-gray-200 shadow-sm rounded-xl">
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    @endphp
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $statusClasses[$invoice->status] }}">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div>
                        ...
                        <p
                            class="text-lg font-semibold {{ $invoice->isOverdue() && $invoice->status !== 'paid' ? 'text-red-600' : 'text-gray-900' }}">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($invoice->items as $item)
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="text-sm font-medium text-gray-900">{{ number_format($itemTotal, 2) }} MT
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                        ...
                        <div class="space-y-2">
                            ...
                            @if ($invoice->paid_amount > 0)
                                ...
                                @if ($invoice->remaining_amount > 0)
                                    <div class="flex justify-between text-sm">
                                        ...
                                        <span class="text-red-600">{{ number_format($invoice->remaining_amount, 2) }}
                                            MT</span>
                                    </div>
                                @endif
                            @endif
                            <div class="pt-2 border-t border-gray-200">
                                <div class="flex justify-between">
                                    ...
                                    <span class="text-xl font-bold text-blue-600">{{ number_format($invoice->total, 2) }}
                                        MT</span>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="relative flex space-x-3">
                                    ...
                                    <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                        <div>
                                            ...
                                            @if ($invoice->quote)
                                                <p class="text-xs text-gray-400">Baseada na cotação
                                                    {{ $invoice->quote->quote_number }}</p>
                                            @endif
                                        </div>
                                        ...
                                        <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                            <div>
                                                <p class="text-sm text-gray-500">
                                                    ...
                                                    @if ($invoice->paid_amount > 0)
                                                        <span
                                                            class="font-medium text-green-600">({{ number_format($invoice->paid_amount, 2) }}
                                                            MT)</span>
                                                    @endif
                                                </p>
                                            </div>
            ...
            <div
                ...
                <form id="emailForm" action="{{ route('invoices.send-email', $invoice) }}" method="POST">
                    ...
                    <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <div
                                class="flex items-center justify-center flex-shrink-0 w-12 h-12 mx-auto bg-purple-100 rounded-full sm:mx-0 sm:h-10 sm:w-10">
