    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-3xl font-bold text-gray-900">Editar Cotação</h1>
                <p class="mt-2 text-gray-600">Cotação #{{ $quote->quote_number }} • Cliente: {{ $quote->client->name }}</p>
            </div>
            <div>
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="date" name="quote_date" id="quote_date"
                                       ...
                                       value="{{ old('quote_date', $quote->quote_date) }}" required>
                                @error('quote_date')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="valid_until" id="valid_until"
                                   ...
                                   value="{{ old('valid_until', $quote->valid_until) }}" required>
                            @error('valid_until')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600">Adicione ou edite itens da cotação</p>
                                </div>
                            </div>
                    ...
                    <div class="p-6 space-y-6">
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="4"
                                      ...
                                      placeholder="Observações sobre a cotação...">{{ old('notes', $quote->notes) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="terms_conditions" id="terms_conditions" rows="4"
                                      class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                      placeholder="Termos e condições da cotação...">{{ old('terms_conditions', $quote->terms_conditions) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="p-6">
                        <div class="space-y-4">
                            <div class="flex items-center justify-between py-2 border-b border-gray-100">
                                ...
                                <span id="subtotalDisplay" class="text-sm font-bold text-gray-900">{{ number_format($quote->subtotal, 2, ',', '.') }} MT</span>
                            </div>
                            <div class="flex items-center justify-between py-2 border-b border-gray-100">
                                <span class="text-sm font-medium text-gray-700">IVA:</span>
                                <span id="taxDisplay" class="text-sm font-bold text-gray-900">{{ number_format($quote->tax_amount, 2, ',', '.') }} MT</span>
                            </div>
                            <div class="flex items-center justify-between px-4 py-3 border-l-4 border-blue-500 rounded-lg bg-gradient-to-r from-blue-50 to-blue-100">
                                ...
                                <span id="totalDisplay" class="text-xl font-bold text-blue-800">{{ number_format($quote->total, 2, ',', '.') }} MT</span>
                            </div>
                        </div>
                        ...
                        <div class="pt-6 mt-6 border-t border-gray-200">
                            <div class="grid grid-cols-2 gap-4">
                                <div class="text-center">
                                    <div class="text-2xl font-bold text-blue-600" id="itemCount">{{ $quote->items->count() }}</div>
                                    <div class="text-xs text-gray-500">Itens</div>
                                </div>
                                <div class="text-center">
                                    <div class="text-2xl font-bold text-green-600" id="avgPrice">{{ $quote->items->isEmpty() ? '0' : number_format($quote->total / $quote->items->count(), 2, ',', '.') }}</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let selectedItems = [
        @foreach($quote->items as $item)
        {
            ...
            complexity: '{{ addslashes($item->complexity_level) }}'
        }{{ !$loop->last ? ',' : '' }}
        @endforeach
    ];
    let products = [];
    ...
    // Renderizar itens selecionados na inicialização
    renderSelectedItems();
    calculateTotals();
