...
<head>
    ...
    <title>{{ session('block_title', 'Acesso Bloqueado') }}</title>
...
<body class="bg-gray-50">
    <div class="flex items-center justify-center min-h-screen px-4">
        <div class="w-full max-w-md">
            ...
            @php
                $reason = session('block_reason', 'unknown');
                ...
                <div class="px-6 pt-8 pb-6 {{ $colors['bg'] }} border-b {{ $colors['border'] }}">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <h1 class="mt-4 text-2xl font-bold {{ $colors['title'] }}">
                            {{ session('block_title', 'Acesso Bloqueado') }}
                        </h1>
                ...
                <div class="px-6 py-6">
                    <div class="p-4 border-l-4 {{ $colors['border'] }} {{ $colors['bg'] }} rounded">
                        <p class="text-sm {{ $colors['text'] }}">
                            {{ session('block_message', 'Sua conta está temporariamente bloqueada.') }}
                        </p>
                    </div>

                    {{-- Informações adicionais baseadas no motivo --}}
                    @if(in_array($reason, ['expired', 'trial_expired']))
                    ...
                    @if(in_array($reason, ['suspended', 'subscription_suspended']))
                        <div class="p-4 mt-6 border border-yellow-200 rounded-lg bg-yellow-50">
                            <div class="flex">
                                ...
                                <div class="text-sm text-yellow-800">
                                    ...
                                    <p class="mt-1">Nossa equipe de suporte está pronta para ajudá-lo a resolver esta situação.</p>
                                </div>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="flex flex-col space-y-3 sm:flex-row sm:space-y-0 sm:space-x-3">
                        ...
                        @if(in_array($reason, ['expired', 'trial_expired', 'cancelled']))
                            <a href="{{ route('billing.plans') }}" 
                               class="inline-flex items-center justify-center px-6 py-3 text-sm font-medium text-white rounded-md {{ $colors['button'] }} focus:outline-none focus:ring-2 focus:ring-offset-2">
                        ...
                        @if(in_array($reason, ['suspended', 'subscription_suspended', 'inactive', 'pending']))
                            <a href="#" 
                               class="inline-flex items-center justify-center px-6 py-3 text-sm font-medium text-white rounded-md {{ $colors['button'] }} focus:outline-none focus:ring-2 focus:ring-offset-2">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="w-full sm:w-auto">
                            @csrf
                            <button type="submit" 
                                    class="w-full px-6 py-3 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                                Sair
                            </button>
                        </form>
                    </div>
            ...
            <p class="mt-6 text-xs text-center text-gray-500">
                ...
                <a href="mailto:suporte@seusite.com" class="text-blue-600 hover:text-blue-700">suporte@seusite.com</a>
            </p>
        </div>
    </div>
</body>
